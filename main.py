...
def crear_producto(producto: dtos.ProductoCreate, db:Session = Depends(get_db)):
    return crud.crear_producto(db=db,producto=producto)

@app.put('/productos/{producto_id}', response_model=dtos.ProductoResponse)
def actualizar_producto(producto_id:int, producto:dtos.ProductoCreate, db:Session = Depends(get_db)):
